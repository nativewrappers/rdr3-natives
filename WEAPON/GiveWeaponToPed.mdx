...
```c
// 0x5E3BDDBCB83F3D84
Hash GIVE_WEAPON_TO_PED(Ped ped, eWeaponHash weaponHash, int ammoCount, BOOL bForceInHand, BOOL bForceInHolster, int attachPoint, BOOL bAllowMultipleCopies, float p7, float p8, eAddItemReason addReason, BOOL bIgnoreUnlocks, float permanentDegradation, BOOL p12);
```
